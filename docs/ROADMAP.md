...
- **Task 002: 프로젝트 구조 및 라우팅 설정** - 우선순위
  - Next.js App Router 기반 전체 라우트 구조 생성
  - 메인 페이지 (`/`), 카테고리 페이지 (`/category/[name]`), TIL 상세 페이지 (`/til/[slug]`), About 페이지 (`/about`) 빈 껍데기 생성
...
- **Task 006: 카테고리 페이지 UI 구현**
  - 카테고리 헤더 (현재 카테고리 표시)
  - TIL 카드 리스트 (메인과 동일 형식)
  - 페이지네이션 UI
  - 더미 데이터로 필터링 UI 검증

- **Task 007: TIL 상세 페이지 UI 구현**
...
- **Task 008: About 페이지 UI 구현**
  - 프로젝트 소개 섹션
  - 기술 스택 소개 섹션
  - Notion CMS 활용 방법 설명

- **Task 009: 반응형 디자인 및 접근성 검증**
  - 모바일 (< 640px) 레이아웃 최적화
