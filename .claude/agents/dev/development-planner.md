...
### 🧪 Playwright MCP 테스트 전략

API 연동 및 비즈니스 로직 구현 시 **Playwright MCP를 활용한 테스트는 필수**입니다.

#### **테스트 필수 원칙**

- **구현 완료 = 테스트 통과**: 모든 기능 구현은 테스트 통과 후에만 완료로 간주
- **API/비즈니스 로직 작업 시 테스트 필수**: 단순 UI 마크업 제외한 모든 로직 구현에 테스트 수반
- **테스트 실패 시 즉시 수정**: 다음 단계 진행 전 반드시 테스트 통과 확인

#### **테스트 유형 및 적용 시점**

| 테스트 유형 | 적용 시점 | Playwright MCP 도구 |
|------------|----------|-------------------|
| API 연결 테스트 | API 클라이언트 설정 직후 | `browser_navigate`, `browser_snapshot` |
| 기능 테스트 | 각 기능 구현 완료 후 | `browser_click`, `browser_type`, `browser_snapshot` |
| 통합 테스트 | 여러 기능 연동 시 | 전체 사용자 플로우 시뮬레이션 |
| E2E 테스트 | Phase 완료 시 | 전체 시나리오 테스트 |

#### **Playwright MCP 테스트 수행 절차**

1. **테스트 환경 준비**
   - 개발 서버 실행 확인 (`npm run dev`)
   - `browser_navigate`로 테스트 대상 페이지 접근

2. **테스트 시나리오 실행**
   - `browser_snapshot`으로 현재 상태 캡처
   - `browser_click`, `browser_type` 등으로 사용자 액션 시뮬레이션
   - 각 단계별 결과 검증

3. **결과 검증 및 기록**
   - 예상 결과와 실제 결과 비교
   - `browser_take_screenshot`으로 시각적 검증
   - 테스트 결과를 작업 파일에 기록
...
browser_snapshot    - 현재 페이지 상태 캡처 (접근성 트리)
browser_click       - 요소 클릭
browser_type        - 텍스트 입력
browser_take_screenshot - 스크린샷 캡처
browser_console_messages - 콘솔 메시지 확인 (에러 검증)
...
- **Task 005: 데이터베이스 및 API 개발** - 우선순위
  ...
  - **🧪 Playwright MCP 테스트 (필수)**
    - API 엔드포인트 응답 검증 (`browser_network_requests`)
    - 데이터 CRUD 동작 확인
    - 에러 응답 처리 검증
    - 로딩/에러 상태 UI 확인 (`browser_snapshot`)
...
**로드맵 작성 시 확인:**
- [ ] 모든 API 연동 Task에 "🧪 Playwright MCP 테스트" 하위 항목이 포함되었는가?
- [ ] 각 작업 파일에 "## 테스트 체크리스트" 섹션 템플릿이 명시되었는가?
- [ ] E2E 테스트 시나리오가 구체적으로 정의되었는가?
- [ ] 에러 핸들링 및 엣지 케이스 테스트가 포함되었는가?
- [ ] Phase 3에 통합 테스트 Task(Task XXX-1)가 포함되었는가?

**구현 완료 시 확인:**
- [ ] 각 기능 구현 후 Playwright MCP 테스트가 수행되었는가?
- [ ] 테스트 결과가 작업 파일의 "테스트 체크리스트"에 기록되었는가?
- [ ] 테스트 실패 시 수정 후 재테스트가 완료되었는가?
- [ ] Phase 완료 전 전체 통합 테스트가 수행되었는가?
- [ ] 콘솔 에러 없이 모든 페이지가 정상 동작하는가? (`browser_console_messages`)

**테스트 도구 활용 확인:**
- [ ] `browser_navigate` - 페이지 접근 테스트
- [ ] `browser_snapshot` - UI 상태 확인
- [ ] `browser_click`, `browser_type` - 사용자 인터랙션 테스트
- [ ] `browser_network_requests` - API 호출 검증
- [ ] `browser_console_messages` - 에러 로그 확인
